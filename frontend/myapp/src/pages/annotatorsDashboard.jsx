...
export default function AnnotatorsDashboard() {
  ...
  const [selectedIds, setSelected]      = useState(new Set());
  const [expandedId, setExpanded]       = useState(null);
  const [menuOpen, setMenuOpen]         = useState(false);   // 3-dot menu
  const [navMenuOpen, setNavMenuOpen]   = useState(false);   // mobile nav hamburger
  const [toasts, setToasts]             = useState([]);
  const [importResult, setImportResult] = useState(null);
  const [formError, setFormError]       = useState("");
  ...
  return (
    <div className="app-root">
      ...
      {/* ════════════════════════════════════════
          NAVBAR  (matches chatbot.jsx navbar)
      ════════════════════════════════════════ */}
      <nav className="navbar">
        ...
        <div className="nav-links">
          ...
          {(isAnnotator || isAdmin) && (
            <Link className="nav-link active" to="/annotator">Annotator</Link>
          )}
        </div>

        <div className="nav-actions">
          {user && (
            <span style={{ fontSize: 13, color: "#5c5880" }}>👋 {user.username}</span>
          )}
          <button
      ...
      <div className={`mobile-menu ${navMenuOpen ? "open" : ""}`}>
        ...
        {(isAnnotator || isAdmin) && (
          <Link className="nav-link active" to="/annotator" onClick={() => setNavMenuOpen(false)}>
            Annotator
          </Link>
        )}
        ...
        <button
          className="nav-link"
          style={{ border: "none", background: "none", cursor: "pointer", textAlign: "left", color: "#e53e3e" }}
          onClick={handleLogout}
        >
          Logout
        </button>
            ...
            <section className="card">
              ...
              <div className="card-body">
                ...
                <div className="grid-2">
                  ...
                  <div className="form-group">
                    <label className="field-label">
                      Keywords / Synonyms <span className="required">*</span>
                    </label>
                    <input
                      className="field-input"
                      placeholder="e.g., truth, integrity, honesty, truthfulness"
